...
export async function up(knex: Knex): Promise<void> {
  ...
  if (!(await knex.schema.hasTable('food'))) {
    await knex.schema.createTable('food', table => {
      ...
      table.integer('saturated_fat').notNullable()
      table.integer('trans_fat').notNullable()
      table.integer('carbonhydrate').notNullable()
  ...
  if (!(await knex.schema.hasTable('food_in_diet'))) {
    await knex.schema.createTable('food_in_diet', table => {
      table.increments('id')
      table.integer('diet_record_id').unsigned().notNullable().references('diet_record.id')
